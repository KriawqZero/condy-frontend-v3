  ...
  return (
    ...
    >
      ...
      <div className='relative max-w-screen-2xl px-4 sm:px-6 lg:px-8 mx-auto flex justify-between items-start pt-6'>
        ...
        {/* Logout Button */}
        {!visitante && (
          <LogoutButton className='w-14 h-14 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center hover:bg-amber-200 transition duration-500 cursor-pointer z-50 relative' />
        )}
      </div>
      ...
      <div className='hidden absolute right-0 bottom-0 opacity-10'>
        <img src='3d_illustration.png' alt='Ilustração de Prédio' className='w-64 lg:w-96' />
      </div>
    </header>
  );
}
